...
describes what users get. Pick a format with `-o`, or let smithsonian choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
smithsonian <command> -o table   # aligned columns for reading
smithsonian <command> -o jsonl   # one JSON object per line, for piping
smithsonian <command> -o json    # a single JSON array
smithsonian <command> -o csv     # spreadsheet friendly
smithsonian <command> -o tsv     # tab-separated
smithsonian <command> -o url     # just the URL column
smithsonian <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
smithsonian <command> --fields id,title,url
```
...
```bash
smithsonian <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
smithsonian <command>            # a table, because this is a terminal
smithsonian <command> | wc -l    # JSONL, because this is a pipe
```
