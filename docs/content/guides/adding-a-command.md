---
title: "Add a command"
description: "Model a real smithsonian record and expose it as a command, a route, and a tool at once."
weight: 10
---

The scaffold ships one example type, `page`. Real work means modelling the
records smithsonian actually serves. You do that in two files, and every surface
updates itself.

## 1. Model the record

In `smithsonian/smithsonian.go`, add a struct for the thing you are fetching
...
type Item struct {
    ...
    Owner string `json:"owner" kit:"link,kind=smithsonian/user"` // an edge to another record
}
...
- `kit:"link,kind=<scheme>/<type>"` marks an outbound edge. It can point at
  another smithsonian type or at another site entirely, which is what lets a
  host walk the graph across tools.

## 2. Declare the operation

In `smithsonian/domain.go`, add an input struct and a handler, then register
it in `Register`:
...
```bash
smithsonian item <id>                 # the command
curl 'localhost:7777/v1/item/<id>'      # the route, under serve
ant get smithsonian://item/<id>       # the URI dereference, via a host
```
